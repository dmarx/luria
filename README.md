...
| `luria index` | regenerates the decision index and per-tag pages from frontmatter |
...
| layer | holds | test |
|---|---|---|
| design principles | standing **values**, numbered and citable | *have we re-derived this more than once?* |
...
are **views**, assembled on a cadence. A file every contribution appends to is a
lock, and its conflicts carry no information
([DP-2](docs/design-principles.md)).
...
mechanism → guarantee ([DP-5](docs/design-principles.md)) — and this package is
the last rung.
...
taste gets re-litigated ([ADR-009](docs/decisions/adr-009-extracted-with-provenance.md)).

Luria runs its own machinery on its own record — the decision index in this repo
is generated by `luria index`, and these files are linted by `luria lint`. That
is not tidiness: it is how the first consumer to hit a bug is this repo.
