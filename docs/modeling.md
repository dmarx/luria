...
| an entry belongs to one or more of a closed set of values, absent meaning one of them | `vocabularies` with `many = true`, `default = ["B"]`, values in `NAME.yaml` | a violation per unknown value |
