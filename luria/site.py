...
`note-properties` plugin renders. The edges are precisely the lineage a
citation graph exists to show, and Quartz's own backlinks say only that a
page was mentioned. Each link is resolved by the same resolver the record
uses everywhere else, then spelled from the vault root.
"""
...
{colors}
plugins:
  # Frontmatter is parsed HERE since Quartz 5 — there is no separate
  # frontmatter transformer — and nothing else reads it. Left out, the YAML
  # renders as a paragraph and every page is untitled: blank search results,
  ...
  - source: "@quartz-community/note-properties"
    ...
    options:
      includeAll: true
      excludedProperties: [{hidden}]
      hidePropertiesView: false
      delimiters: "---"
      language: yaml
    order: 5
    layout: {{position: beforeBody, priority: 35, display: all}}
...
// A many-valued property is a list of documents, each a code and a title —
// too long to read run together with commas, so one per line.
.note-properties .note-properties-list > .note-properties-separator {
  display: none;
}
.note-properties .note-properties-list > .note-properties-text {
  display: block;
}
"""
...
def _page(text: str, path: Path, cfg, typed, known: dict[str, str],
          report: Report, prefix: str, landing: bool) -> str:
    """One page as Quartz should read it: a composed frontmatter — the keys
    Quartz uses, the title with its code, and the record's facts as
    properties (`site_page`) — over the body minus the heading the title now
    says for it.

    Every page gets one, generated views included: a page Quartz cannot
    title is a blank search result and a graph node labelled with its path
    (dmarx/anthology-of-the-sota#271). The landing page is the exception; it
    is named by `landing_page`, after the site."""
    yaml_text, body = split_frontmatter(text)
    meta = parse_frontmatter(text)[0] if yaml_text is not None else {}
    code = edges.code_of(path)
    alias = _alias(path, cfg)
    props, unresolved = site_page.properties(
        meta, path,
        outbound=typed.outbound(code) if code else (),
        inbound=typed.inbound(code) if code else (),
        known=known, cfg=cfg, prefix=prefix)
    if props:
        report.described += 1
    if unresolved:
        report.unplaced.append(f"{cfg.rel(path)} → unresolved in frontmatter: "
                               + " ".join(unresolved))
    front = {k: meta[k] for k in site_page.QUARTZ_KEYS
             if k in meta and k != "title"}
    if not landing and (title := site_page.title_of(meta, body, alias)):
        front = {"title": title, **front}
        body = site_page.without_title(body, title)
    if alias:
        held = front.get("aliases") or []
        held = held if isinstance(held, list) else [held]
        front["aliases"] = [*held, alias] if alias not in held else held
    front.update(props)
    if not front:
        return text
    dumped = yaml.safe_dump(front, sort_keys=False, allow_unicode=True,
                            width=1 << 20, default_flow_style=False)
    return "---\n" + dumped + "---\n\n" + body.lstrip("\n")
...
def stage(out: Path, cfg=None, nested: bool = True,
    ...
    `stage_nested` — and `prefix` is where that record is mounted, which a
    link spelled from the vault root has to include."""
    cfg = cfg or current()
    out = out.resolve()
    content = out / "content"
    if content.exists():
    ...
    # frontmatter.
    typed = edges.graph()
    known = site_page.titles()

    for path in pages:
        text = path.read_text(encoding="utf-8")
        dest_rel = destination(path, cfg)
        text = _page(text, path, cfg, typed, known, report, prefix,
                     landing=dest_rel.as_posix() == "index.md")
...
def stage_nested(content: Path, cfg, report: Report) -> None:
    ...
    **The child's own `include_records` is not honoured.** One level is a
    ...
    for record in records:
        ...
        report.described += child_report.described
