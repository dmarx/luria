...
# Quartz's own local graph, as the layout writes it when a project has not
# configured one of its own. Retuned for the content column: Quartz's defaults
# are set for a 320px rail, and in a column twice that width they leave the
# neighbourhood huddled in the middle. Depth stays at 1 — depth 2 on a record
# this densely cross-cited renders a hairball with the current page lost in it,
# which is a picture of nothing.
QUARTZ_GRAPH_COMPONENT = """\
    Component.Graph({
      localGraph: {
        depth: 1,
        scale: 1.05,
