...
<!-- mention-ok-file: ADR-tmpabcde, DP-018 — the other two specimens, quoted as the evidence for the decision. Named, not cited: this document says nothing about whether either resolves. The DP code here is the next principle number, so `unresolved-ok` was a scheduled failure -->
