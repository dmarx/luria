...
summary: >-
  ...
  export; set, that one curated picture replaces Quartz's own graph plugin on
  every page. Opt-in, and the inverse of the generated lineage graph: one graph the
...
was** — Quartz's own graph plugin is written `enabled: false` in the generated
config and the element is inserted above each document's own heading. Unset,
nothing changes.
