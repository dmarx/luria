...
def test_writes_both_reports(tmp_path):
    ...
    assert [p.name for p in written] == ['pending-decisions.md', 'reference-status.md']
...
def test_reference_report_links_the_flagged_document(project):
    ...
    import os.path
    rel = os.path.relpath(current().schemes['ADR'].dir / 'ADR-012.md', current().reports)
...
def test_reference_report_counts_match_the_scan():
    ...
    text = reports.reference_status()
    assert f"**{reports._n(len(rows), 'document')} cited without acknowledgement.**" in text
...
def test_outputs_land_in_the_configured_reports_dir():
    from luria.config import current
    assert set(reports.outputs()) == {current().reports / 'reference-status.md', current().reports / 'pending-decisions.md'}
...
def test_a_gitignored_report_dir_is_not_stale(tmp_path, monkeypatch):
    ...
    config.reset()
    adr_index.run()
    assert (tmp_path / 'build' / 'doc-reports' / 'reference-status.md').exists(), 'an ignored view is still written'
