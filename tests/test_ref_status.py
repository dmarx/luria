...
def test_line_annotation_excuses_its_own_line(project):
    docs, result = scan(project, 'per ADR-012 <!-- inactive-ok: ADR-012 — deliberate -->\n')
...
def test_line_annotation_excuses_the_line_below(project):
    docs, result = scan(project, '<!-- inactive-ok: ADR-012 — history -->\nper ADR-012\n')
    assert ref_status.flagged(result, docs) == []

def test_a_bare_annotation_does_not_reach_across_a_blank_line(project):
    docs, result = scan(project, '<!-- inactive-ok: ADR-012 -->\n\nper ADR-012\n')
    assert [c.line for _, sites, _ in ref_status.flagged(result, docs) for c in sites] == [3]

def test_a_standalone_block_annotation_governs_what_it_introduces(project):
    docs, result = scan(project, '<!-- inactive-ok-block: ADR-012 -->\n\nper ADR-012\nagain ADR-012\n\nbut not here, ADR-012\n')
    assert [c.line for _, sites, _ in ref_status.flagged(result, docs) for c in sites] == [6]

def test_block_annotation_covers_its_paragraph(project):
    docs, result = scan(project, 'first ADR-012 <!-- inactive-ok-block: ADR-012 -->\nsecond ADR-012\n\nnext paragraph ADR-012\n')
    assert [c.line for _, sites, _ in ref_status.flagged(result, docs) for c in sites] == [4]

def test_file_annotation_covers_the_whole_document(project):
    docs, result = scan(project, '<!-- inactive-ok-file: ADR-012 — supersession history -->\n\nfirst ADR-012\n\n\nmuch later ADR-012\n')
    assert ref_status.flagged(result, docs) == []
    assert ref_status.acknowledged_count(result, docs) == 2

def test_annotation_works_in_a_code_comment(project):
    docs, result = scan(project, '// shaped by ADR-012  // inactive-ok: ADR-012 — shipped\n', name='thing.ts')
    assert ref_status.flagged(result, docs) == []
...
def test_bare_numbers_are_rejected(project):
    ...
    assert ref_status.flagged(result, docs)
    assert any(('no document code' in s for s in ref_status.stale_annotations(result, docs)))

def test_mixed_bare_number_is_rejected(project):
    docs, result = scan(project, '<!-- inactive-ok: ADR-012, 20 -->\n')
    assert any(('bare number' in s for s in ref_status.stale_annotations(result, docs)))

def test_unknown_code_is_reported(project):
    docs, result = scan(project, '<!-- inactive-ok: ADR-999 -->\n')
    assert any(('unknown document' in s for s in ref_status.stale_annotations(result, docs)))

def test_annotation_for_an_active_document_is_stale(project):
    docs, result = scan(project, '<!-- inactive-ok: ADR-001 -->\nper ADR-001\n')
    assert any(('Active now' in s for s in ref_status.stale_annotations(result, docs)))

def test_annotation_with_no_matching_reference_is_stale(project):
    docs, result = scan(project, '<!-- inactive-ok: ADR-012 -->\n')
    assert any(('no longer applies' in s for s in ref_status.stale_annotations(result, docs)))
...
def test_spread_prefers_distinct_files(tmp_path):
    ...
    sites = [ref_status.Citation(a, 1, 'ADR-001'), ref_status.Citation(a, 2, 'ADR-001'), ref_status.Citation(b, 9, 'ADR-001')]
    assert [c.line for c in ref_status._spread(sites, 2)] == [1, 9]

def test_report_never_fails_the_build():
    assert ref_status.run() is None
...
def test_unresolved_ok_retires_one(project):
    docs, result = scan(project, "<!-- unresolved-ok: ADR-777 — another project's -->\nADR-777\n")
...
def test_unresolved_ok_naming_a_real_document_is_malformed(project):
    docs, result = scan(project, '<!-- unresolved-ok: ADR-001 — deliberate -->\nADR-001\n')
...
def test_a_code_inside_a_url_is_not_a_citation(project):
    docs, result = scan(project, 'see [their ADR](https://github.com/o/r/blob/main/docs/ADR-777.md)\n')
    assert result.dangling == {}
