...
def configure(root: Path, value: str, base_url: str = "example.test/rec",
    ...
    # Parsed and re-dumped rather than appended to. Under TOML a second
    # `[luria.site]` table concatenated onto the text; a YAML mapping does
    # not, so a fixture that pastes `site:` on the end either duplicates the
    # key or silently replaces whatever the base config put there (ADR-101).
    doc = yaml.safe_load(path.read_text()) or {}
    site_cfg = dict(doc.get("site") or {})
    site_cfg["graph"] = value
    if base_url:
        site_cfg["base_url"] = base_url
    if depth is not None:
        site_cfg["graph_depth"] = depth
    doc["site"] = site_cfg
...
def _graph_plugin(out: Path) -> dict:
    """Quartz's own graph plugin, as the generated config declares it. Parsed
    rather than string-matched: `enabled` is the key that decides whether the
    project's configured graph REPLACES this one or stacks on top of it, and a
    substring check would pass on a commented-out block (ADR-101 moved this
    from a spliced-out TSX component to one key)."""
    import yaml
    config = yaml.safe_load((out / "quartz.config.yaml").read_text())
    return next(p for p in config["plugins"]
                if p["source"].endswith("/graph"))
...
def test_without_the_key_quartzs_graph_is_what_the_layout_keeps(project):
    ...
    site.stage(out)
    assert _graph_plugin(out)["enabled"] is True
...
def test_the_configured_graph_replaces_quartzs(project):
    ...
    assert _graph_plugin(out)["enabled"] is False, \
        "both graphs would stack above every document's first paragraph"
